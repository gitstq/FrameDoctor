...
describe('FrameDoctor Core', () => {
  ...
  describe('Health Score Calculation', () => {
    ...
    it('should never return negative score', () => {
      const issues: DiagnosticIssue[] = Array(100).fill(null).map((_, i) => ({
        ruleId: `test-${i}`,
  ...
  describe('Category Scores', () => {
    it('should return all categories', () => {
      ...
      assert.ok(scores.every(s => s.score === 100));
      assert.ok(scores.every(s => s.issueCount === 0));
    });
  ...
  describe('Rules Registry', () => {
    ...
    it('should filter by category', () => {
      ...
      assert.ok(secRules.every(r => r.category === 'security'));
    });

    it('should filter by severity', () => {
      const rules = getRulesForFramework('vue');
      const errorsOnly = filterBySeverity(rules, 'error');
      assert.ok(errorsOnly.every(r => r.severity === 'error' || r.severity === 'warn'));
    });
  });
